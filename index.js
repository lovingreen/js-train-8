...
function createArray(...elements) {
  if (elements.length > 0) {
    ...
    console.log("Помилка: жодного елемента не передано.");
    return [];
  }
...
function findElementIndexes(arr, element) {
  if (Array.isArray(arr)) {
    const indexOfArray = arr.indexOf(element);
    const lastIndexOfArray = arr.lastIndexOf(element);
    let arrForElements = [];
    if (indexOfArray !== -1 && indexOfArray !== -1) {
      arrForElements.push(indexOfArray);
      if (lastIndexOfArray !== indexOfArray) {
        arrForElements.push(lastIndexOfArray);
      }
    }
    return arrForElements;
  } else {
    return [];
  }
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if (target >= arr.length && target < 0 && start >= arr.length && start < 0 && end >= arr.length && end < 0) {
...
function customFill(arr, value, start = 0, end = arr.length) {
  ...
  if (!Number.isInteger(start) || !Number.isInteger(end) || start < 0 || start > arr.length || end < start || end > arr.length) {
    return false;
  }
...
function customUnshift(arr, ...elements) {
  if (!Array.isArray(arr)) {
    return "Перший аргумент має бути масивом";
  }
